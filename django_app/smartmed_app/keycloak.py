...
class KeycloakConnect:
    ...
    def get_pat_token(self, access_token, path_info):
        uri, scope = RESOURCE_MAPPING[path_info]
        payload = {
    ...
    def get_resource_ticket(self, context_token, path_info):
        uri, scope = RESOURCE_MAPPING[path_info]
        payload = [
            {
                "resource_id": uri,
    ...
    def has_context_access(self, token, path_info):
        context_token = self.get_pat_token(token, path_info)
        print("Got context token", context_token)
        if context_token is not None:
            resource_ticket = self.get_resource_ticket(context_token, path_info)
